...
NHLogger *
PipelineAccessor::get_logger() const
{
    return m_ppu->m_logger;
}
...
{
    return nhd::is_debug_on(m_ppu->m_debug_flags, NHD_DBG_PALETTE);
}
...
{
    return nhd::is_debug_on(m_ppu->m_debug_flags, NHD_DBG_OAM);
}
...
{
    return nhd::is_debug_on(m_ppu->m_debug_flags, NHD_DBG_PATTERN);
}
