...
DMC::DMC(DMCDMA &o_dmc_dma, NHLogger *i_logger)
    : m_dmc_dma(o_dmc_dma)
...
DMC::tick_timer()
{
    ...
    if (m_timer.tick())
    {
        ...
        {
            NH_ASSERT_FATAL(m_logger, "Invalid bits remaining value {}",
                            m_bits_remaining);
        }
...
DMC::set_timer_reload(Byte i_index)
{
    ...
    if (i_index >= 0x10)
    {
        NH_ASSERT_FATAL(m_logger, "Invalid dmc timer reload index: {}",
                        i_index);
        return;
    }
